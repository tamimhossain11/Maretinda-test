...
const ThumbnailPreview = ({ url }: { url?: string | null }) => {
  ...
  const imageUrl = url.startsWith('http') ? url : 
    url.startsWith('/') ? `${import.meta.env.VITE_MEDUSA_BACKEND_URL || 'http://localhost:9000'}${url}` :
    url.startsWith('blob:') ? url : // For preview URLs
    `${import.meta.env.VITE_MEDUSA_BACKEND_URL || 'http://localhost:9000'}/${url}`
